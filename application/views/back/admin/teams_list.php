<div class="panel-body" id="demo_s">
	...
	<div id="toolbar">
		<a class="btn btn-primary btn-labeled fa fa-plus-circle" onclick="ajax_modal('add','Add Team','<?php echo translate('successfully_added!'); ?>','teams_add','')">Create Teams</a>
  </div>
  <?php } ?>
	<table id="demo-table" class="table table-striped" data-pagination="true" data-show-refresh="true" data-search="true" data-toolbar="#toolbar">
		...
		<tbody >
			...
			<tr>
				...
				<img class="img-md" src="<?php echo base_url(); ?>uploads/teams_image/<?php echo $row['logo_thumb']; ?>" /> <?php
				} else {	?>
				...
				<td class="text-right">
					...
					<?php if($this->session->userdata('role') != 3){ ?>
					<a class="btn btn-success btn-xs btn-labeled fa fa-wrench" data-toggle="tooltip" 
						onclick="ajax_modal('edit','<?= translate('edit_teams'); ?>','<?= translate('successfully_edited!'); ?>','teams_edit','<?php echo $row['teams_id']; ?>')" data-original-title="Edit" data-container="body">Edit</a>
						
					<a onclick="delete_confirm('<?= $row['teams_id']; ?>','<?= translate('really_want_to_delete_this?'); ?>')" class="btn btn-danger btn-xs btn-labeled fa fa-trash" data-toggle="tooltip" data-original-title="Delete" data-container="body">Delete</a>
					<?php } ?>
				</td>
			</tr>
			<?php
