	...
	function ajax_load(url,id,type){
		var list = $('#'+id);console.debug(list);
		$.ajax({
	...
	function ajax_set_full(type,title,noty,form_id,id){
		//full_form(title,noty,form_id);console.log(base_url+'index.php/'+user_type+'/'+module+'/'+type+'/'+id,'list','form');
	...
	function delete_confirm2(id,msg){
		msg = '<div class="modal-title">'+msg+'</div>';
		bootbox.confirm(msg, function(result) {
			if (result) {
				ajax_load(base_url+'index.php/'+user_type+'/'+module+'/'+dlt_cont_func2+'/'+id,'list','delete');
				$.activeitNoty({
					type: 'danger',
					icon : 'fa fa-check',
					message : dss,
	...
	function modal_form(title,noty,form_id){
		console.log(form_id);
		if(form_id=='address_edit'){
			var btnClass = 'change_address';
		}else{
			var btnClass = 'enterer';
		}
		bootbox.dialog({
			title: title,
